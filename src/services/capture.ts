...
function notifyFileMoved(
	...
	const move = db.transaction(() => {
		const indexed = indexFiles(db, vaultRoot, [oldPath, newPath]);
		const current = db
...
export function notifyFilesChanged(
	...
	try {
		const indexed = indexFiles(db, vaultRoot, filePaths);
		return {
...
export function notifyFileChanged(
	...
	if (previousFilePath) {
		try {
			return notifyFileMoved(db, vaultRoot, previousFilePath, filePath);
		} catch (error) {
			return {
				action: 'error',
				filePath,
	...
	return notifyFilesChanged(db, vaultRoot, [filePath]).results[0] as NotifyFileChangedResult;
}
