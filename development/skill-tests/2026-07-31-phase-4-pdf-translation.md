...
> 阶段六复查：本文件的命令、JSON 和文件副作用属于受控 PyMuPDF 脚本执行证据；它不执行
> 真实翻译 Agent、用户确认或 MCP 通知，故不扩展为翻译工作流端到端结论。双语 Schema、模板和
> 完整性规则另由阶段六校验器与 `tests/assets/read-pdf-extraction.test.ts` 持续检查。
