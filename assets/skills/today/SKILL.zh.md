...
8. **收集画像数据**（用于生成用户摘要）
   - 从上述步骤中汇总：
     - 所有项目的 domain、status、章节覆盖率
     - 各项目下知识笔记的 status 分布（draft/revise/mastered）
   - 读取知识笔记中的以下段落（仅概要，不全文）：
     - 习题解答：解题记录和对错
     - 复习记录：问答表现和得分
     - 个人补充：用户自己的理解和批注
...
基于步骤一收集的画像数据，生成一段叙述性的用户知识掌握画像：

1. **综合分析**以下维度：
   - 各项目的推进进度和完成度
   - 各领域的知识掌握深度（基于笔记 status 分布和复习表现）
   - 薄弱环节（复习中出错的概念、长期停留在 draft 的笔记）
   - 个人理解的独到之处（来自个人补充）

2. **生成画像描述**：一段 100-200 字的中文叙述，包含：
   - 当前学习重心和各领域进展
   - 已扎实掌握的知识点
   - 需要加强的薄弱环节
   - 整体学习节奏评价
...
3. **写入记忆**：
   ```
   memory_log(slot_key="profile:summary", content="<生成的画像描述>")
   ```

> 注意：若项目和笔记数据不足以生成有意义的画像（如无活跃项目或笔记），跳过此步骤。
