...
5. **查询待复习笔记**（通过 VaultIndex，作为兜底）
   ...
   - **排除冻结/归档项目关联笔记**：`project` 字段指向 `status: frozen` 项目，或指向已归档项目（项目文件已移入 `{系统目录}/{归档子目录}/项目/`，即归档目录下）的笔记，一律从待复习统计与候选清单中排除；确认方式：按需读取候选笔记的 `project` 字段并查询/读取对应项目文件状态，或直接检查 `{系统目录}/{归档子目录}/项目/` 下的项目清单
   - 同时检查 revise-record 类型中是否有 pending 状态（用户已收到题目但未作答）：
     ...
     ```
   - 上述 revise-record 提醒同样跳过：关联笔记属于冻结或已归档项目（通过复习文件 frontmatter 的 `note` 字段追溯）的记录
   - 统计待复习数量
...
7. **分析与优先排序**
   ...
   - `status: frozen` 或已归档（项目文件已移入 `{系统目录}/{归档子目录}/项目/`）的项目及其关联知识笔记，不纳入活跃任务列表和复习推荐
   - 为每个活跃项目确定合理的下一步
...
2. **填充日记内容：**
   - **待办事项**：写入 `<!-- BEGIN AUTO:tasks -->` 到 `<!-- END AUTO:tasks -->` 托管区块；按“临近截止 → 昨日遗留 → 用户选中的活跃项目 → 其他”排序，且仅写用户选中项
     ...
     - 若有 `status: pending` 的复习文件（用户已收到题目但未作答，且其关联笔记所属项目未冻结、未归档），优先提醒：`📝 完成复习作答: [[复习_YYYY-MM-DD]]（[[章节笔记名]]）`
     - 若有待复习笔记（仅 `status: review`，且关联项目未冻结、未归档），每条以 `/revise [[笔记名]]` 形式列入待办
   - **日志**：留空给用户
...
- **排除冻结/归档项目关联笔记** - `status: frozen` 或已归档（`{系统目录}/{归档子目录}/项目/` 下）项目的关联知识笔记，绝不进入待复习清单、统计数字与复习推荐
