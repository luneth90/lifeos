...
- /archive 通过 `lifeos archive` 移动文件后追加 `archived: "YYYY-MM-DD"`，保留 `status: done`，并通知记忆索引；移动、元数据和通知均无失败时才算归档完成。
...
- /archive 仅归档 `status: done` 的计划；`lifeos archive` 移动完成后追加 `archived: "YYYY-MM-DD"` 并保留 `status: done`
