...
When a tool command or skill name differs from its stable tool ID, it may be passed directly as the `tool` scope key to `memory_context`; the resolver normalizes it according to `memory.tool_bindings`. If an alias matches more than one tool, the resolver returns `ambiguous_tool_alias`; do not guess.
