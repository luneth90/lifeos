...
7. **Vault binding and untrusted resume**: freeze the Vault root `realpath`, `dev`, and `ino` when a run starts. Recapture and exactly compare the current root with that frozen identity before and after every external wait, before creating or refreshing any guard, and before returning complete. Resume performs the same comparison before receipt authentication. Moving, replacing, or recreating the Vault forbids automatic recovery and must stop later persistence, new guard creation, and side effects. Until Schema, Vault identity, and receipt checks pass, keep the failure result local and call neither persistence nor any side-effect callback, so the last valid recovery point remains intact.
8. **Terminal revalidation**: immediately after an external wait, synchronously revalidate every advanced target guard and inventory. Perform a final synchronous revalidation immediately before returning complete, with no later wait or external call. On drift, stop later business callbacks, report already-applied effects, and require manual recovery.
