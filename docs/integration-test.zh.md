...
#### 7.3.2 调用 memory_bootstrap 工具（验证显式 Layer 0 入口）
...
```bash
cd tmp/test-mcp
# 在 Claude Code 会话中先触发 memory_bootstrap，再按需调用其他工具
claude "先调用 memory_bootstrap，再调用 memory_query 搜索'测试'"
```

**Codex：**
```bash
cd tmp/test-mcp
codex "先调用 memory_bootstrap，再调用 memory_query 搜索'测试'"
```

- [ ] Agent 成功调用 `memory_bootstrap` 并返回包含 `_layer0` 的结果
