...
```bash
lifeos help           # 显示帮助信息
lifeos --version      # 显示版本号（应与 package.json 一致，当前为 1.4.0）
lifeos --help         # 同 help
...
#### 7.3.2 调用 memory_query 工具（验证自动启动）

> `memory_startup` 已内部化，不再作为 MCP 工具暴露。MCP Server 会在首次工具调用时自动触发。
> 这里通过调用 `memory_query` 验证首次调用会自动完成启动并返回 `_layer0` 字段。
...
```bash
cd tmp/test-mcp
# 在 Claude Code 会话中触发 memory_query（交互式，需人工观察）
# 首次工具调用会自动触发 memory_startup
claude "调用 memory_query 搜索'测试'"
```

**Codex：**
```bash
cd tmp/test-mcp
codex "调用 memory_query 搜索'测试'"
```

- [ ] Agent 成功调用 `memory_query` 并返回包含 `_layer0` 的结果（自动启动已触发）
