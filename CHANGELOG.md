...
- 复习链路彻底排除冻结/归档项目关联知识笔记：today 待复习扫描与推荐、TaskBoard 待复习面板、Dashboard 待复习 KPI 与表格、Layer 0 复习提醒、revise 默认候选，均不再包含 `project` 字段指向 `status: frozen` 或已归档（项目文件移入 `{系统目录}/{归档子目录}/项目/`）项目的笔记
- 修复归档项目关联待复习笔记绕过过滤：归档项目文件不在 `vault_index` 索引中（system 目录被排除），旧过滤因查不到项目状态而失效；TaskBoard 新增 `loadArchivedProjectKeys` 直接扫描归档项目目录，按文件名/标题/id/相对路径多 key 匹配排除
- Dashboard 待复习面板按项目路径与 frontmatter title 双通道匹配排除；`project` 链接无法解析（标题形式/纯字符串/指向不存在文件）的笔记同样不显示，避免冻结/归档项目笔记漏出

### 改进

- today/revise 技能指令、lifecycle 状态机文档、lifeos-rules 全局硬约束（zh/en）同步"冻结或已归档项目关联笔记不进入复习链路"表述
- 新增测试覆盖：归档项目关联笔记排除、标题形式链接匹配、无 `project` 字段笔记保留、归档目录缺失容错
