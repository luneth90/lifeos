...
import { SCHEMA_VERSION, initDb } from '../../src/db/schema.js';
...
describe('initDb', () => {
    ...
    `).run(
		...
		const row = db
			.prepare('SELECT * FROM vault_index WHERE file_path = ?')
			.get('20_项目/test-project.md') as {
			file_path: string;
			title: string;
			type: string;
			status: string;
    ...
    `).run(
		...
		const rows = db
			...
			.all('trigger') as Array<{ file_path: string }>;
    ...
    `).run(
			...
			'active',
			'[]',
			'deleteme unique term xyz',
		);

		const before = db
			.prepare('SELECT file_path FROM vault_fts WHERE vault_fts MATCH ?')
			.all('"unique term xyz"') as Array<{ file_path: string }>;
    ...
    `).run('format:commit-msg', 'concise messages', '2026-03-26T10:00:00Z');
		...
		const row = db
			.prepare('SELECT * FROM memory_items WHERE slot_key = ?')
			.get('format:commit-msg') as {
			manual_flag: number;
			status: string;
			source: string;
		};
    ...
    `).run('40_知识/ch1.md', 'Chapter 1', 'note', 'draft', '[[VGT学习]]');
		...
		const row = db
			.prepare('SELECT project FROM vault_index WHERE file_path = ?')
			.get('40_知识/ch1.md') as { project: string };
