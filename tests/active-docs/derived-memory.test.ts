...
describe('upsertRule', () => {
  ...
  it('inserts a new rule and returns created', () => {
    ...
    const row = db
      ...
      .get('tool:editor') as Record<string, unknown> | undefined;
...
describe('cleanupExpiredItems', () => {
  ...
  it('marks expired items as expired status', () => {
    ...
    const row = db
      .prepare('SELECT status FROM memory_items WHERE slot_key = ?')
      .get('temp:slot') as { status: string } | undefined;
  ...
  it('dryRun returns count without deleting', () => {
    ...
    const row = db
      .prepare('SELECT status FROM memory_items WHERE slot_key = ?')
      .get('dry:slot') as { status: string } | undefined;
    expect(row?.status).toBe('active');
  });
});
